/**
 * Invoke pick-daily over HTTP. Newer Supabase CLIs dropped `functions invoke`.
 *
 * Usage:
 *   node scripts/pick.mjs
 *   node scripts/pick.mjs --force
 *   node scripts/pick.mjs --today --force
 *   node scripts/pick.mjs --target=2026-08-23 --force
 *
...
if (secret.startsWith("sb_publishable_")) {
  fail("Esa es la publishable key. Para pick hace falta la Secret key (sb_secret_...).");
}

const jwtRole = decodeJwtRole(secret);
if (jwtRole === "anon") {
  fail("Pegaste la anon/publishable. Para pick hace falta la Secret key (sb_secret_...).");
}
...
if (argv.includes("--today")) body.target = "today";
if (argv.includes("--tomorrow")) body.target = "tomorrow";
const targetArg = argv.find((a) => a.startsWith("--target="));
if (targetArg) body.target = targetArg.slice("--target=".length);

const url = `${urlBase}/functions/v1/pick-daily`;
const headers = { "Content-Type": "application/json" };

if (secret.startsWith("sb_secret_")) {
  // New keys are not JWTs. Authorization: Bearer sb_secret_... is rejected.
